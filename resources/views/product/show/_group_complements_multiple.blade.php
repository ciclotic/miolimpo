            ...
            <div class="col-md-1 col-1 mt-2">
                <div class="form-check">
                    @if ($groupProduct->pivot->price > 0)
                        <input class="form-check-input" type="checkbox" {{ ($complementProduct->pivot->selected)? 'checked' : '' }} name="products-to-complements-selected[{{ $complementProduct->id }}]" onchange="changeComplementFromGroupProduct({{ $groupProduct->id }}, {{ $complementProduct->id }}, '{{ number_format($complementProduct->price, 2, ',', '.') }}')">
                    @else
